...
上面已经清晰了 Taro 转快应用的生命周期方式，但同时我们也发现了还有好些快应用的生命周期（如 `APP` 的生命周期 `onShow`、`onHide`、`onError`等等），Taro 并没有支持到（原因可能是还没有跟上快应用的迭代变化，或者认为我们就是不需要.）。所以虽然有些功能社区暂时还不支持，但是我们业务场景上就要用了，那我们也不能反馈社区后只等社区吧，我们完全也是可以自己封装办到的。
...
其实发现编译后的源码很好理解，`exports.default` 导出的是 Taro 编译后的一个对象。那么假如我们对这个对象做重新赋值，并添加上我们想要添加的原生生命周期方法，那是不是就可以满足了。
...
简单说明：我们只要做到最后我们编译后生成的代码中，赋值我们要添加的生命周期方法 `quickappLifecycle` 即可。那么查看 Taro 源码发现其 astProcess 的生成模板都是固定的，那么就动点小手脚，在模板顶部插入一个变量
...
以上的拦截都是比较粗暴的覆盖形式，可以做更细的优化就不展开了，同时其他的页面级别的生命周期拦截方式也是类似的，也不继续展开了。

## 全局的错误监控

以上我们已经基本摸清了 Taro 的生命周期原理，那错误监控是每一个应用都应该做的，以下就是简单的业务实践。我们分别对小程序和快应用做了全局的错误监控，建议将错误日志对接到内部的错误日志平台上，平台可以做很多的数据分析，以及预警机制，来保障我们的应用稳定。
...
摸清 Taro 生命周期实现原理后，能够很大程度上帮助我们减少在用 Taro 转快应用的过程中，在生命周期上踩得坑。同时我们也可以考虑下如何做快应用的页面级别的用户行为埋点了，下一章会展开介绍。